...
export function Logo({ className = '', size = 'md', variant = 'dark' }: LogoProps) {
  ...
  const subColor = variant === 'light' ? 'text-purple-200' : 'text-purple-700';

  return (
    <div className={`flex items-center gap-2.5 font-bold tracking-tight select-none ${className}`}>
      {/* Stylized RWS Tech Mark */}
      <div
        className={`${sizeClasses.icon} relative flex items-center justify-center rounded-xl bg-gradient-to-tr from-purple-700 via-purple-600 to-indigo-600 text-white shadow-md shadow-purple-600/20 ring-1 ring-purple-500/30 overflow-hidden group`}
      >
        {/* Geometric Tech Glyph Accent */}
        <div className="absolute inset-0 bg-gradient-to-br from-white/20 via-transparent to-black/20" />
        <span className="relative z-10 font-black tracking-tighter">
          R<span className="text-purple-200">W</span>S
        </span>
        <div className="absolute -bottom-1 -right-1 w-3 h-3 bg-purple-300 rounded-full blur-[2px] opacity-70" />
      </div>

      <div className="flex flex-col leading-none">
        <div className="flex items-center gap-1">
          <span className={`${sizeClasses.text} ${textColor} font-black tracking-tight`}>
            RWS<span className="text-purple-600">.</span>TECH
          </span>
          {/* Subtle accent chevron inspired by enterprise tech aesthetic */}
          <span className="text-purple-600 font-mono font-bold text-xs">/&gt;</span>
        </div>
